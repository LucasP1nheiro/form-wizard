...
const SelectFields = ({ id }: { id: string }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <div className="space-y-4">
      <Input
        placeholder="Add a label for this select field"
        onChange={(e) => handleLabelChange(e.target.value)}
        value={props.label}
      />
      <div className="flex items-center justify-between">
        <Label>Is this select field required?</Label>
        <Switch
      ...
      >
        <div className="flex flex-col gap-3 w-full">
          <Input
            placeholder="Add a new option for this select field"
            {...register('text')}
            className={cn('w-auto', {
